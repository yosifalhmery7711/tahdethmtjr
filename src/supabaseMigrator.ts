...
// Initialize a dedicated source Firebase App & Firestore for migration.
// This ensures we ALWAYS read from the actual applet's development database (where the 196 products are)
// instead of reading from the newly configured, empty production database defined in .env!
const sourceApp = getApps().find(app => app.name === 'source-migration-app') || initializeApp({
...
const sourceDb = appletConfig.firestoreDatabaseId
